  ...
  return (
    <Html>
      <div className="z-10 flex flex-col justify-center items-center w-screen h-screen bg-[#e5dddb] absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <img className="h-56 " src={loading} />
        <p className="text-[#8b6565] text-3xl font-bold">Đang tải model 3D</p>
        <p className="text-[#8b6565] mt-2 text-xl font-semibold">
    ...
    >
      ...
      {showModelInfo && (
        <div className="relative w-full px-10 py-4 text-white text-xl">
          ...
          className="flex justify-center gap-2 hover:text-xl group text-lg z-50 w-full absolute pb-2 top-0 left-1/2 transform -translate-x-1/2 transition-all duration-300 text-white cursor-pointer">
          <FontAwesomeIcon
            icon={faCaretDown}
            className=""
          />
          <p className="text-sm text-center hidden group-hover:inline group-hover:text-base transition-all duration-300">Đóng</p>
          </div>
          ...
          <div className="text-white z-30 relative flex flex-col gap-2 justify-center py-4">
            <h4 className="pt-0 font-semibold text-2xl">
              {heritage.name}
            </h4>
            <p className="text-base overflow-auto max-h-56 pr-2 scrollbar">
              <style>
                {`
...
export default UserModelViewerOverlay;
