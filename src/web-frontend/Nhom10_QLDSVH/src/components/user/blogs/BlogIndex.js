...
import {
  ...
  Subheading,
} from "components/user/misc/Headings.js";
...
const PostContainer = styled.div`
  ...
  ${(props) =>
    ...
    `}
`;
const Post = tw.div`cursor-pointer flex flex-col rounded-lg shadow-lg overflow-hidden`;
...
const LoadMoreButton = tw(
  PrimaryButton
)`mt-16 mx-auto bg-amber-500 hover:bg-amber-600`;
const ShortenButton = tw(
  PrimaryButton
)`mt-16 mx-auto bg-red-500 hover:bg-red-600`;
...
const Heading = tw(
  SectionHeading
)`text-gray-900 mb-0 mt-3 text-lg text-left mr-3`;
...
export default ({
  heading = (
    <>
      Danh sách<span tw="text-primary-500"> địa điểm</span>
    </>
  ),
  description = `Bạn có thêm xem các di sản văn hóa thuộc từng điểm điểm hiện có tại Di sản văn hóa Lâm Đồng, hãy bấm vào địa điểm để xem danh sách di sản tương ứng với mỗi địa điểm.`,
}) => {
                ...
                <>
                  ...
                  <PostContainer key={index} featured={featured}>
                    <Post
                      ...
                      as="a"
                      href={
                        "/all-heritage/" + "by-location/" + location.urlslug
                      }
                    >
                      {isEmptyOrSpaces(location.image_url) ? (
                        <Image imageSrc={PostDefault}>
                          <CardRatingContainer>
                            <CardRating>{location.heritage_count}</CardRating>
